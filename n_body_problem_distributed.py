...
def calculate_accelerations(positions, masses):
    """
    Calculate the acceleration of each body in the system
    positions: a slice of the positions of all bodies, a 2D numpy array of shape (n, 2)
    masses: a slice of the masses of all bodies, a 1D numpy array of shape (n,)
...
def main():
    ...
    while True:
        ...
        if comm.rank == 0:
            ...
            ax.scatter(positions[:, 0], positions[:, 1], s=masses.squeeze() * 100, c=colors)
            ax.set_xlim(-100, 100)
            ax.set_ylim(-100, 100)
